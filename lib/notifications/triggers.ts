...
export async function runTriggers(supabase: SupabaseClient, now = new Date()) {
  ...
  const { data: overdue } = await supabase
    .from("rent_schedule")
    .select("lease_id, amount_due, amount_collected, tenant:tenant_id(full_name), property:property_id(address)")
    .neq("invoice_status", "Paid")
    .lt("due_date", todayStr);

  const byLease = new Map<string, { arrears: number; tenant: string | null; property: string | null }>();
  ...
  for (const [leaseId, info] of byLease) {
    ...
    await raise(supabase, staff, {
      ...
      lease_id: leaseId,
      message: `${label} is in rent arrears of ${amount}.`,
      emailSubject: `Rent overdue: ${label}`,
      emailHtml: `<p>${label} is in rent arrears of ${amount}.</p>`,
    });
    overdueCount++;
  }
